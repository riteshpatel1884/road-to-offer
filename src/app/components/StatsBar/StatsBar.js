...
export default function StatsBar({ total, done, dsa, da, genai, backend, core, aptitude, daysPassed }) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
      <div className="col-span-2 sm:col-span-5 bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg p-4">
        <div className="flex justify-between text-xs text-[var(--text-muted)] mb-2">
          <span>Overall completion</span>
          <span className="text-[var(--text-primary)]">{done}/{total} topics &nbsp;·&nbsp; {pct}%</span>
        </div>
        <div className="w-full h-1.5 bg-[var(--bg-elevated)] rounded-full overflow-hidden">
          <div className="h-full bg-[var(--accent-green)] rounded-full transition-all duration-500" style={{ width: `${pct}%` }} />
        </div>
        <div className="flex justify-between text-xs text-[var(--text-muted)] mt-2">
          <span>Time elapsed</span>
          <span className="text-[var(--text-primary)]">{Math.min(daysPassed, 45)}/45 days &nbsp;·&nbsp; {dayPct}%</span>
        </div>
        <div className="w-full h-1.5 bg-[var(--bg-elevated)] rounded-full overflow-hidden mt-1">
          <div className="h-full bg-[var(--accent-blue)] rounded-full transition-all duration-500" style={{ width: `${dayPct}%` }} />
        </div>
      </div>
      ...
      <StatCard label="Completed"    value={done}       color="text-[var(--accent-green-text)]" />
...
function StatCard({ label, value, color }) {
  return (
    <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg p-3 flex flex-col gap-1">
